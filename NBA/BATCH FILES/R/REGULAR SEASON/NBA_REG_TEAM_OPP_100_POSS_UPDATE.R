...
# This will update: NBA_TEAM_REG_OPP_100_POSS.rda
...
load(file.path(team_fp,"NBA_TEAM_REG_OPP_100_POSS.rda"))
...
latest_team_opp_100_poss <- scrape_data_in_batches(nba_urls)

# Drops rows that have Rk that contain NA and drop the entire column afterwards
latest_team_opp_100_poss <- latest_team_opp_100_poss %>%
  filter(!is.na(Rk)) %>%
  select(-Rk)

# Remove '*' from the 'Team' column
latest_team_opp_100_poss$Team <- gsub("\\*", "", latest_team_opp_100_poss$Team)
...
latest_team_opp_100_poss <- latest_team_opp_100_poss %>%
  ...
  mutate( From = as.numeric(substr(latest_team_opp_100_poss$Season,1,4)),
          To = as.numeric(substr(latest_team_opp_100_poss$Season,6,9))) %>%
...
# Read in standings data to join into latest_team_opp_100_poss
...
# Filter out the most recent season's data from nba_team_reg_opp_100_poss
nba_team_reg_opp_100_poss <- nba_team_reg_opp_100_poss %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_reg_opp_100_poss
nba_team_reg_opp_100_poss <- bind_rows(nba_team_reg_opp_100_poss, latest_team_opp_100_poss) %>%
  arrange(`Team Name`,desc(Season))
