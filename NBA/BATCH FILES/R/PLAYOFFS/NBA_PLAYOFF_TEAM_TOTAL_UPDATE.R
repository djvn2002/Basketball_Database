...
load(file.path(team_fp,"NBA_TEAM_PLAYOFF_TOTAL.rda"))
...
recent_team_ply_total <- scrape_data_in_batches(nba_urls)

# Drops rows that have Rk that contain NA and drop the entire column afterwards
recent_team_ply_total <- recent_team_ply_total %>%
  filter(!is.na(Rk)) %>%
  select(-Rk)
...
# Read in standings data to join into recent_team_ply_total
...
# Filter out the most recent season's data from nba_team_ply_total
nba_team_ply_total <- nba_team_ply_total %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_ply_total
nba_team_ply_total <- bind_rows(nba_team_ply_total, recent_team_ply_total) %>%
  arrange(`Team Name`,desc(Season))
