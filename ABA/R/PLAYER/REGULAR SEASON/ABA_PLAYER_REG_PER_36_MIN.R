...
scrape_all_urls_selenium <- function(aba_urls, remDr, batch_size = 100, restart_every = 500) {
  ...
  write_csv(bind_rows(results), file.path(player_fp, "ABA_PLAYER_REG_PER_36_MIN_partial.csv"))
...
aba_reg_per_36_parallel <- with_progress({
...
# Cleaning up tables
aba_reg_per_36_min <- convert_to_numeric(aba_reg_per_36_parallel) %>%
  filter(Rk != "Rk") %>%
  select(-Rk)
...
aba_reg_per_36_min <- aba_reg_per_36_min %>%
...
# Save the final aba_reg_per_36_min table to a RDA file
save(aba_reg_per_36_min,file = file.path(player_fp,"ABA_PLAYER_REG_PER_36_MIN.rda"))

# Display message to confirm save
print("aba_reg_per_36_min table has been saved to ABA_PLAYER_REG_PER_36_MIN.rda")

# Delete the partial RDA file
file.remove(file.path(player_fp,"ABA_PLAYER_REG_PER_36_MIN_partial.csv"))