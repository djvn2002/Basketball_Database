...
# NBA_TEAM_PLAYOFF_OPP_TOTAL.rda
...
load(file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_TOTAL.rda"))
...
scrape_data_in_batches <- function(nba_urls, batch_size = 30) {
  ...
  for (i in 1:num_batches) {
    ...
    write_csv(bind_rows(results), file.path(team_fp,"NBA_TEAM_PLAYOFFS_OPP_TOTAL_partial.csv"))
...
recent_team_ply_opp_total <- scrape_data_in_batches(nba_urls)

# Drops rows that have Rk that contain NA and drop the entire column afterwards
recent_team_ply_opp_total <- recent_team_ply_opp_total %>%
  filter(!is.na(Rk)) %>%
  select(-Rk)
...
# Read in standings data to join into recent_team_ply_opp_total
...
# Filter out the most recent season's data from nba_team_ply_opp_total
nba_team_ply_opp_total <- nba_team_ply_opp_total %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_ply_opp_total
nba_team_ply_opp_total <- bind_rows(nba_team_ply_opp_total, recent_team_ply_opp_total) %>%
  arrange(`Team Name`,desc(Season))

# Save per game data frame to a rda file
save(nba_team_ply_opp_total,file = file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_TOTAL.rda"))

# Display message to confirm save
print("nba_team_ply_opp_total table has been saved to NBA_TEAM_PLAYOFF_OPP_TOTAL.rda")

# Delete the partial RDA file
file.remove(file.path(team_fp,"NBA_TEAM_PLAYOFFS_OPP_TOTAL_partial.csv"))