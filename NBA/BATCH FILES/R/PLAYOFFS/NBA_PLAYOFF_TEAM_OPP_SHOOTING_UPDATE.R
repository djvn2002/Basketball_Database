...
load(file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_SHOOTING.rda"))
...
scrape_table_selenium <- function(remDr, url, max_retries = 3) {
  for (i in 1:max_retries) {
    tryCatch({
      ...
      webElem <- remDr$findElement(using = "css", "#div_shooting-opponent")
...
scrape_all_urls_selenium <- function(nba_urls, remDr, batch_size = 100, restart_every = 500) {
  ...
  write_csv(bind_rows(results), file.path(team_fp, "NBA_TEAM_PLAYOFF_OPP_SHOOTING_partial.csv"))
...
nba_ply_opp_shooting_parallel <- with_progress({
...
recent_team_ply_opp_shooting <- nba_ply_opp_shooting_parallel %>%
  rename(Rk = Var.1,
         Team = Var.2,
         G = Var.3,
         MP = Var.4,
  ...
  filter(Rk != "Rk" & Team != "League Average" &  MP != "") %>%
  select(-Rk) %>%
...
recent_team_ply_opp_shooting <- recent_team_ply_opp_shooting %>%
...
# Read in standings data to join into recent_team_ply_opp_shooting
...
# Filter out the most recent season's data from nba_team_ply_opp_shooting
nba_team_ply_opp_shooting <- nba_team_ply_opp_shooting %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_ply_opp_shooting
nba_team_ply_opp_shooting <- bind_rows(nba_team_ply_opp_shooting, recent_team_ply_opp_shooting) %>%
  arrange(`Team Name`,desc(Season))

# Save per game data frame to a rda file
save(nba_team_ply_opp_shooting,file = file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_SHOOTING.rda"))

# Display message to confirm save
print("nba_team_ply_opp_shooting table has been saved to NBA_TEAM_PLAYOFF_OPP_SHOOTING.rda")

# Delete the partial RDA file
file.remove(file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_SHOOTING_partial.csv"))