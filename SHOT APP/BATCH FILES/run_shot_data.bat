...
if %ERRORLEVEL% neq 0 (
    echo WNBA Shots Script failed or data was skipped at %time% on %date% >> %log_file%
) else (
...
if %ERRORLEVEL% neq 0 (
    echo NBA Shots Script failed or data was skipped at %time% on %date% >> %log_file%
) else (
...
if %ERRORLEVEL% neq 0 (
    echo MNCAA Shots Script failed or data was skipped at %time% on %date% >> %log_file%
) else (
...
if %ERRORLEVEL% neq 0 (
    echo WNCAA Shots Script failed or data was skipped at %time% on %date% >> %log_file%
) else (
