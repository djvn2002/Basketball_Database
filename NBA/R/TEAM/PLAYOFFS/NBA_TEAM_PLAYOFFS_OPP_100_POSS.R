...
nba_team_ply_opp_100_poss <- nba_ply_opp_100_poss_parallel %>%
  filter(!is.na(Rk)) %>%
  select(-Rk) %>%
...
nba_team_ply_opp_100_poss <- nba_team_ply_opp_100_poss %>%
  ...
  mutate( From = as.numeric(substr(nba_team_ply_opp_100_poss$Season,1,4)),
          To = as.numeric(substr(nba_team_ply_opp_100_poss$Season,6,9))) %>%
...
# Read in standings data to join into nba_team_ply_opp_100_poss
