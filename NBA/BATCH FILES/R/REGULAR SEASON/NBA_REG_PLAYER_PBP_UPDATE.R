...
# This will update: NBA_PLAYER_REG_PBP.rda
...
load(file.path(player_fp,"NBA_PLAYER_REG_PBP.rda"))
...
scrape_table_selenium <- function(remDr, url, max_retries = 3) {
  for (i in 1:max_retries) {
    tryCatch({
      ...
      webElem <- remDr$findElement(using = "css", "#div_pbp")
...
scrape_all_urls_selenium <- function(nba_urls, remDr, batch_size = 100, restart_every = 500) {
  ...
  write_csv(bind_rows(results), file.path(player_fp, "NBA_PLAYER_REG_PBP_partial.csv"))
...
nba_reg_pbp_parallel <- with_progress({
...
# Cleaning up tables
latest_pbp <- convert_to_numeric(nba_reg_pbp_parallel)

# Rename columns and drop Rk
latest_pbp <- latest_pbp %>%
  rename(Rk = Var.1,
         Player = Var.2,
         Age = Var.3,
         G = Totals,
...
# Filter out the most recent season's data from nba_reg_pbp
nba_reg_pbp <- nba_reg_pbp %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_reg_pbp
nba_reg_pbp <- bind_rows(nba_reg_pbp, latest_pbp) %>%
  arrange(`Team Name`,desc(Season), Player)

# Save the final nba_reg_pbp table to a RDA file
save(nba_reg_pbp,file = file.path(player_fp,"NBA_PLAYER_REG_PBP.rda"))

# Display message to confirm save
print("nba_reg_pbp table has been saved to NBA_PLAYER_REG_PBP.rda")

# Delete the partial RDA file
file.remove(file.path(player_fp,"NBA_PLAYER_REG_PBP_partial.csv"))