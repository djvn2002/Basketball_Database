...
load(file.path(team_fp,"NBA_TEAM_PLAYOFF_SHOOTING.rda"))
...
recent_team_ply_shooting <- nba_ply_shooting_parallel %>%
  rename(Rk = Var.1,
         Team = Var.2,
         G = Var.3,
         MP = Var.4,
...
recent_team_ply_shooting <- recent_team_ply_shooting %>%
...
# Read in standings data to join into recent_team_ply_shooting
...
# Filter out the most recent season's data from nba_team_ply_shooting
nba_team_ply_shooting <- nba_team_ply_shooting %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_ply_shooting
nba_team_ply_shooting <- bind_rows(nba_team_ply_shooting, recent_team_ply_shooting) %>%
  arrange(`Team Name`,desc(Season))
