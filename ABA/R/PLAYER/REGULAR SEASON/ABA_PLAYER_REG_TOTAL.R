...
# This file will contain scrape code for player total traditional stats throughout ABA history
...
scrape_data_in_batches <- function(aba_urls, batch_size = 30) {
  ...
  for (i in 1:num_batches) {
    ...
    write_csv(bind_rows(results), file.path(player_fp,"ABA_PLAYER_REG_TOTAL_partial.csv"))
...
aba_reg_total <- scrape_data_in_batches(aba_urls)

# Filter out rows where Rk column is NA
aba_reg_total <- aba_reg_total %>%
  filter(!is.na(Rk))

# Load the ABA roster data and league info
...
# Save the final aba_reg_total table to a RDA file
save(aba_reg_total,file = file.path(player_fp,"ABA_PLAYER_REG_TOTAL.rda"))

# Display message to confirm save
print("aba_reg_total table has been saved to ABA_PLAYER_REG_TOTAL.rda")

# Delete the partial RDA file
file.remove(file.path(player_fp,"ABA_PLAYER_REG_TOTAL_partial.csv"))