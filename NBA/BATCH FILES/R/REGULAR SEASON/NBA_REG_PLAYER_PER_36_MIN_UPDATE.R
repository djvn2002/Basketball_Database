...
# This will update: NBA_PLAYER_REG_PER_36_MIN.rda
...
load(file.path(player_fp,"NBA_PLAYER_REG_PER_36_MIN.rda"))
...
scrape_table_selenium <- function(remDr, url, max_retries = 3) {
  for (i in 1:max_retries) {
    tryCatch({
      ...
      webElem <- remDr$findElement(using = "css", "#div_per_minute")
...
scrape_all_urls_selenium <- function(nba_urls, remDr, batch_size = 100, restart_every = 500) {
  ...
  write_csv(bind_rows(results), file.path(player_fp, "NBA_PLAYER_REG_PER_36_MIN_partial.csv"))
...
nba_reg_per_36_parallel <- with_progress({
...
# Cleaning up tables
latest_36_min <- convert_to_numeric(nba_reg_per_36_parallel) %>%
  filter(Rk != "Rk") %>%
  select(-Rk)
...
# Filter out the most recent season's data from nba_reg_per_36_min
nba_reg_per_36_min <- nba_reg_per_36_min %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_reg_per_36_min
nba_reg_per_36_min <- bind_rows(nba_reg_per_36_min, latest_36_min) %>%
  arrange(`Team Name`,desc(Season), Player)

# Save the final nba_reg_per_36_min table to a RDA file
save(nba_reg_per_36_min,file = file.path(player_fp,"NBA_PLAYER_REG_PER_36_MIN.rda"))

# Display message to confirm save
print("nba_reg_per_36_min table has been saved to NBA_PLAYER_REG_PER_36_MIN.rda")

# Delete the partial RDA file
file.remove(file.path(player_fp,"NBA_PLAYER_REG_PER_36_MIN_partial.csv"))
