...
# Load in current player playoff totals
load(file.path(player_fp,"NBA_PLAYER_PLAYOFF_TOTAL.rda"))
...
recent_playoff_total <- scrape_data_in_batches(nba_urls)

# Filter out rows where Rk column is NA
recent_playoff_total <- recent_playoff_total %>%
  filter(!is.na(Rk)) %>%
  select(-Rk, -URL)
...
# Filter out the most recent season's data from nba_playoff_total
nba_playoff_total <- nba_playoff_total %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_playoff_total
nba_playoff_total <- bind_rows(nba_playoff_total, recent_playoff_total) %>%
  arrange(`Team Name`,desc(Season), Player)
