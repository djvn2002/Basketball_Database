...
load(file.path(team_fp,"NBA_TEAM_PLAYOFF_ADVANCED.rda"))
...
recent_team_ply_adv <- scrape_data_in_batches(nba_urls)

# Drops rows that have Rk that contain NA and drop the entire column afterwards
recent_team_ply_adv <- recent_team_ply_adv %>%
  rename(
    Rk = V1,
    Team = V2,
    Age = V3,
...
# Read in standings data to join into recent_team_ply_adv
...
# Filter out the most recent season's data from nba_team_ply_adv
nba_team_ply_adv <- nba_team_ply_adv %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_ply_adv
nba_team_ply_adv <- bind_rows(nba_team_ply_adv, recent_team_ply_adv) %>%
  arrange(`Team Name`,desc(Season))
