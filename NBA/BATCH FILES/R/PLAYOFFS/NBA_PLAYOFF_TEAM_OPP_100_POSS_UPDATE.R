...
# NBA_TEAM_PLAYOFF_OPP_100_POSS.rda
...
load(file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_100_POSS.rda"))
...
scrape_table_selenium <- function(remDr, url, max_retries = 3) {
  for (i in 1:max_retries) {
    tryCatch({
      ...
      webElem <- remDr$findElement(using = "css", "#div_per_poss-opponent")
...
scrape_all_urls_selenium <- function(nba_urls, remDr, batch_size = 100, restart_every = 500) {
  ...
  write_csv(bind_rows(results), file.path(team_fp, "NBA_TEAM_PLAYOFF_OPP_100_POSS_partial.csv"))
...
nba_ply_opp_100_poss_parallel <- with_progress({
...
recent_team_ply_opp_100_poss <- nba_ply_opp_100_poss_parallel %>%
  filter(!is.na(Rk)) %>%
  select(-Rk) %>%
...
recent_team_ply_opp_100_poss <- recent_team_ply_opp_100_poss %>%
...
# Read in standings data to join into recent_team_ply_opp_100_poss
...
# Filter out the most recent season's data from nba_team_ply_opp_100_poss
nba_team_ply_opp_100_poss <- nba_team_ply_opp_100_poss %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_ply_opp_100_poss
nba_team_ply_opp_100_poss <- bind_rows(nba_team_ply_opp_100_poss, recent_team_ply_opp_100_poss) %>%
  arrange(`Team Name`,desc(Season))

# Save per game data frame to a rda file
save(nba_team_ply_opp_100_poss,file = file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_100_POSS.rda"))

# Display message to confirm save
print("nba_team_ply_opp_100_poss table has been saved to NBA_TEAM_PLAYOFF_OPP_100_POSS.rda")

# Delete the partial RDA file
file.remove(file.path(team_fp,"NBA_TEAM_PLAYOFF_OPP_100_POSS_partial.csv"))