...
load(file.path(team_fp,"NBA_TEAM_REG_100_POSS.rda"))
...
scrape_table <- function(url) {
  ...
  for (i in 1:max_retries) {
    tryCatch({
      ...
      # Extract the table with ID #div_per_poss-team
      table <- webpage %>%
        html_node("#div_per_poss-team") %>%
        html_table()
...
scrape_data_in_batches <- function(nba_urls, batch_size = 30) {
  ...
  for (i in 1:num_batches) {
    ...
    write_csv(bind_rows(results), file.path(team_fp,"NBA_TEAM_REG_100_POSS_partial.csv"))
...
latest_team_100_poss <- scrape_data_in_batches(nba_urls)

# Drops rows that have Rk that contain NA and drop the entire column afterwards
latest_team_100_poss <- latest_team_100_poss %>%
  filter(!is.na(Rk)) %>%
  select(-Rk)

# Remove '*' from the 'Team' column
latest_team_100_poss$Team <- gsub("\\*", "", latest_team_100_poss$Team)
...
latest_team_100_poss <- latest_team_100_poss %>%
  ...
  mutate( From = as.numeric(substr(latest_team_100_poss$Season,1,4)),
          To = as.numeric(substr(latest_team_100_poss$Season,6,9))) %>%
...
# Read in standings data to join into latest_team_100_poss
...
# Filter out the most recent season's data from nba_team_reg_100_poss
nba_team_reg_100_poss <- nba_team_reg_100_poss %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_reg_100_poss
nba_team_reg_100_poss <- bind_rows(nba_team_reg_100_poss, latest_team_100_poss) %>%
  arrange(`Team Name`,desc(Season))

# Save per game data frame to a rda file
save(nba_team_reg_100_poss,file = file.path(team_fp,"NBA_TEAM_REG_100_POSS.rda"))

# Display message to confirm save
print("nba_team_reg_100_poss table has been saved to NBA_TEAM_REG_100_POSS.rda")

# Delete the partial RDA file
file.remove(file.path(team_fp,"NBA_TEAM_REG_100_POSS_partial.csv"))