...
# This will update: NBA_TEAM_REG_OPP_TOTAL.rda
...
load(file.path(team_fp,"NBA_TEAM_REG_OPP_TOTAL.rda"))
...
scrape_table <- function(url) {
  ...
  for (i in 1:max_retries) {
    tryCatch({
      ...
      # Extract the table with ID #div_totals-opponent
      table <- webpage %>%
        html_node("#div_totals-opponent") %>%
        html_table()
...
scrape_data_in_batches <- function(nba_urls, batch_size = 30) {
  ...
  for (i in 1:num_batches) {
    ...
    write_csv(bind_rows(results), file.path(team_fp,"NBA_TEAM_REG_OPP_TOTAL_partial.csv"))
...
latest_team_opp_total <- scrape_data_in_batches(nba_urls)

# Drops rows that have Rk that contain NA and drop the entire column afterwards
latest_team_opp_total <- latest_team_opp_total %>%
  filter(!is.na(Rk)) %>%
  select(-Rk)

# Remove '*' from the 'Team' column
latest_team_opp_total$Team <- gsub("\\*", "", latest_team_opp_total$Team)
...
latest_team_opp_total <- latest_team_opp_total %>%
  ...
  mutate( From = as.numeric(substr(latest_team_opp_total$Season,1,4)),
          To = as.numeric(substr(latest_team_opp_total$Season,6,9))) %>%
...
# Read in standings data to join into latest_team_opp_total
...
# Filter out the most recent season's data from nba_team_reg_opp_total
nba_team_reg_opp_total <- nba_team_reg_opp_total %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_reg_opp_total
nba_team_reg_opp_total <- bind_rows(nba_team_reg_opp_total, latest_team_opp_total) %>%
  arrange(`Team Name`,desc(Season))

# Save per game data frame to a rda file
save(nba_team_reg_opp_total,file = file.path(team_fp,"NBA_TEAM_REG_OPP_TOTAL.rda"))

# Display message to confirm save
print("nba_team_reg_opp_total table has been saved to NBA_TEAM_REG_OPP_TOTAL.rda")

# Delete the partial RDA file
file.remove(file.path(team_fp,"NBA_TEAM_REG_OPP_TOTAL_partial.csv"))