...
# NBA_PLAYER_PLAYOFF_ADVANCED.rda
...
# Load in player advanced
load(file.path(player_fp,"NBA_PLAYER_PLAYOFF_ADVANCED.rda"))
...
recent_playoff_advanced <- scrape_data_in_batches(nba_urls)

# Filter out Rk and then drop unnecessary columns
recent_playoff_advanced <- recent_playoff_advanced %>%
  filter(!is.na(Rk)) %>%
...
nba_duplicates <- recent_playoff_advanced %>%
...
# Filter out the most recent season's data from nba_playoff_advanced
nba_playoff_advanced <- nba_playoff_advanced %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_playoff_advanced
nba_playoff_advanced <- bind_rows(nba_playoff_advanced, recent_playoff_advanced) %>%
  arrange(`Team Name`,desc(Season), Player)
