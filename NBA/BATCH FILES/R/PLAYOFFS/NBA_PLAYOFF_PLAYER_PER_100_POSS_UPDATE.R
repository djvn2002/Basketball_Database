...
# NBA_PLAYER_PLAYOFF_PER_100_POSS.rda
...
# Load playoff pbp
load(file.path(player_fp,"NBA_PLAYER_PLAYOFF_PER_100_POSS.rda"))
...
# Cleaning up tables
recent_playoff_per_100_poss <- convert_to_numeric(nba_playoff_per_100_parallel) %>%
  filter(Rk != "Rk") %>%
  select(-Rk)
...
recent_playoff_per_100_poss <- recent_playoff_per_100_poss %>%
...
nba_duplicates <- recent_playoff_per_100_poss %>%
...
# Filter out the most recent season's data from nba_playoff_per_100_poss
nba_playoff_per_100_poss <- nba_playoff_per_100_poss %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_playoff_per_100_poss
nba_playoff_per_100_poss <- bind_rows(nba_playoff_per_100_poss, recent_playoff_per_100_poss) %>%
  arrange(`Team Name`,desc(Season), Player)
