...
# This will update: NBA_TEAM_REG_SHOOTING.rda
...
load(file.path(team_fp,"NBA_TEAM_REG_SHOOTING.rda"))
...
latest_team_shooting <- scrape_data_in_batches(nba_urls)

# Rename columns and cleaning them for consistency
latest_team_shooting <- latest_team_shooting %>%
  rename(Rk = V1,
         Team = V2,
         G = V3,
         MP = V4,
...
latest_team_shooting$Team <- gsub("\\*", "", latest_team_shooting$Team)
...
latest_team_shooting <- latest_team_shooting %>%
  ...
  mutate( From = as.numeric(substr(latest_team_shooting$Season,1,4)),
          To = as.numeric(substr(latest_team_shooting$Season,6,9))) %>%
...
# Read in standings data to join into latest_team_shooting
...
# Filter out the most recent season's data from nba_team_reg_shooting
nba_team_reg_shooting <- nba_team_reg_shooting %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_team_reg_shooting
nba_team_reg_shooting <- bind_rows(nba_team_reg_shooting, latest_team_shooting) %>%
  arrange(`Team Name`,desc(Season))
