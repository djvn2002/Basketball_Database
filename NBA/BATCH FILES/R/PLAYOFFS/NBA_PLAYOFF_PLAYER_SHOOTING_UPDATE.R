...
# NBA_PLAYER_PLAYOFF_SHOOTING.rda
...
# Load playoff shooting
load(file.path(player_fp,"NBA_PLAYER_PLAYOFF_SHOOTING.rda"))
...
scrape_table_selenium <- function(remDr, url, max_retries = 3) {
  for (i in 1:max_retries) {
    tryCatch({
      ...
      webElem <- remDr$findElement(using = "css", "#div_playoffs_shooting")
...
scrape_all_urls_selenium <- function(nba_urls, remDr, batch_size = 100, restart_every = 500) {
  ...
  write_csv(bind_rows(results), file.path(player_fp, "NBA_PLAYER_PLAYOFF_SHOOTING_partial.csv"))
...
nba_playoff_shooting_parallel <- with_progress({
...
# Cleaning up tables
recent_playoff_shooting <- convert_to_numeric(nba_playoff_shooting_parallel) %>%
  select(-Var.8, -Var.15, -Var.22, -Var.25, -Var.28, -Var.31)

# Rename the columns and drop Rk
recent_playoff_shooting <- recent_playoff_shooting %>%
  rename(Rk = Var.1,
         Player = Var.2,
         Age = Var.3,
         G = Var.4,
...
nba_duplicates <- recent_playoff_shooting %>%
...
# Filter out the most recent season's data from nba_playoff_shooting
nba_playoff_shooting <- nba_playoff_shooting %>%
  filter(Season != most_recent_season_formatted)

# Bind the new latest season data with the filtered nba_playoff_shooting
nba_playoff_shooting <- bind_rows(nba_playoff_shooting, recent_playoff_shooting) %>%
  arrange(`Team Name`,desc(Season), Player)

# Save the final nba_playoff_shooting table to a RDA file
save(nba_playoff_shooting,file = file.path(player_fp,"NBA_PLAYER_PLAYOFF_SHOOTING.rda"))

# Display message to confirm save
print("nba_playoff_shooting table has been saved to NBA_PLAYER_PLAYOFF_SHOOTING.rda")

# Delete the partial RDA file
file.remove(file.path(player_fp,"NBA_PLAYER_PLAYOFF_SHOOTING_partial.csv"))